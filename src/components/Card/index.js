...
export default function Card({ image, title, price, id, isAdded, isFavourite, onClickAdd, onFavorite }) {
   const [isAddedProduct, setIsAddedProduct] = React.useState(isAdded);
   const [isAddedFavourite, setIsFavouriteProduct] = React.useState(isFavourite);

   const handleClickPlus = () => {
      setIsAddedProduct(!isAddedProduct);
      onClickAdd({ image, title, price, id, isAdded: !isAddedProduct, isFavourite: isAddedFavourite });
   }

   const handleClickFavourite = () => {
      setIsFavouriteProduct(!isAddedFavourite)
      onFavorite({ image, title, price, id, isAdded: isAddedProduct, isFavourite: !isAddedFavourite })
   }
   ...
   return (
      <div className={styles.card}>
         ...
         <div className={styles.card__actions}>
            <button onClick={handleClickFavourite} type="button" className={`${styles.card__icon} ${styles.card__icon_favorite} ${isAddedFavourite ? styles.active : ""}`}> {/* styles.card__icon_favorite */}
               <svg width="14" height="13" viewBox="0 0 22 19" fill="none" xmlns="http://www.w3.org/2000/svg">
         ...
         <div className={styles.card__info}>
            <div className={styles.card__meta}>
               <div className={styles.card__subtitle}>Цена:</div>
               <span className={styles.card__price}>{price} руб.</span>
            </div>
            <button onClick={handleClickPlus} type="button" className={`${styles.card__button} ${isAddedProduct ? styles.card__button_active : ""}`}>
